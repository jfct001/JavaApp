...
public class MapGraph {
	...
	// reset the distance information in each node !.
	private void resetMap() {
		for (GeographicPoint p : pointNodeMap.keySet()) {
//			pointNodeMap.get(p).setActualDistance(0.0);
			pointNodeMap.get(p).setDistance(Double.MAX_VALUE);
		}
	}
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			curNode= toExplore.remove();
			num++;
			 // hook for visualization
			nodeSearched.accept(curNode.getLocation());
			if (!visited.contains(curNode)) {
				visited.add(curNode);
				if (curNode.equals(endNode)) break;
				Set<MapEdge> edgeLink = curNode.getEdges();
				for (MapEdge link : edgeLink) {
					linkDistance = link.getLength();
					nextNode = link.getOtherNode(curNode);
					if (nextNode.getDistance() > curNode.getDistance() + linkDistance) {
						nextNode.setDistance(curNode.getDistance() + linkDistance);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		startNode.setActualDistance(start.distance(goal));
		startNode.setDistance(0.0 + startNode.getActualDistance());
		toExplore.add(startNode);
		MapNode curNode = null;
		MapNode nextNode = null;
		...
		double curFakeDistance = 0.0;
		
		int num = 0;

		while (!toExplore.isEmpty()) {
			curNode= toExplore.remove();
			curFakeDistance = curNode.getDistance() - curNode.getActualDistance();
			num++;
			 // hook for visualization
			nodeSearched.accept(curNode.getLocation());
			if (!visited.contains(curNode)) {
				...
				for (MapEdge link : edgeLink) {
					linkDistance = link.getLength();
					nextNode = link.getOtherNode(curNode);
					nextNode.setActualDistance(nextNode.getLocation().distance(endNode.getLocation()));
					
					newDistance = linkDistance + curFakeDistance+ nextNode.getActualDistance();
					if (nextNode.getDistance() > newDistance) {
